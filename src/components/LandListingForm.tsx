...
export default function LandListingForm({ onSubmit, onBack, initialData }: LandListingFormProps) {
  ...
  // Parse local storage draft
  const localDraft = useMemo(() => {
    try {
      const raw = localStorage.getItem('realagents_land_draft');
      if (raw) {
        const parsed = JSON.parse(raw);
        if (parsed && (parsed.state || parsed.address || parsed.askingPrice || parsed.salePrice || parsed.landDetails?.landSize)) {
          return parsed;
        }
      }
    } catch (e) {
      console.warn("Failed to parse local land draft", e);
    }
    return null;
  }, []);

  // Filter cloud drafts matching Land
  const matchingCloudDrafts = useMemo(() => {
    return drafts.filter(d => d.propertyCategory === 'Land' || d.type === 'Land');
  }, [drafts]);

  // Combine them into a list of importable drafts
  const availableDrafts = useMemo(() => {
    const list = [];
    if (localDraft) {
      list.push({
        id: 'local_autosave',
        title: localDraft.title || (localDraft.landDetails?.landSize ? `${localDraft.landDetails.landSize} SQM Saved Progress` : 'In-Progress Auto-Saved Draft'),
    ...
    matchingCloudDrafts.forEach(c => {
      const mappedFormData = {
        propertyCategory: 'Land',
        listingType: c.listingType || 'Sale',
        ...
        landDetails: {
          landSize: String(c.landDetails?.landSize || c.sizeSqm || ''),
          landSizeUnit: c.landDetails?.landSizeUnit || 'sqm',
          landUse: c.landDetails?.landUse || '',
          topography: c.landDetails?.topography || '',
          isEstatePlot: c.landDetails?.isEstatePlot || !!c.estateName,
          estateName: c.landDetails?.estateName || c.estateName || '',
          infrastructure: c.landDetails?.infrastructure || c.amenities || [],
          locationPin: c.landDetails?.locationPin || c.googlePinLink || '',
          surveyPlanFileName: c.landDetails?.surveyPlanFileName || '',
          surveyPlanUrl: c.landDetails?.surveyPlanUrl || '',
        },
      ...
      list.push({
        id: c.id,
        title: c.title || `${c.sizeSqm || 0} SQM Land in ${c.location || 'Unknown'}`,
        source: 'Cloud Draft',
        data: mappedFormData,
            ...
            >
              {importedDraftId ? (
                ...
                availableDrafts.length > 0 && (
                  <div className="bg-amber-100 dark:bg-amber-950/40 border-2 border-amber-500 text-amber-800 dark:text-amber-200 p-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 font-bold text-xs uppercase tracking-wider shadow-brutal-xs mb-4">
                    <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      localStorage.removeItem('realagents_land_draft');
                      if (onSubmit) {
                        await onSubmit({ ...formData, propertyCategory: 'Land', isDraft: false });
                      }
                      setShowPublishModal(false);
                    } catch (err) {
                      console.error("Publish land listing error:", err);
                    } finally {
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      console.error("Save land draft error:", err);
                    } finally {
                      setIsSubmitting(false);
                    }
                  }}
